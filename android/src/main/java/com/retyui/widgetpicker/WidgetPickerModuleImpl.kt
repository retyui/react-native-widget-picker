        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val appWidgetManager: AppWidgetManager =
                    reactContext.getSystemService(AppWidgetManager::class.java)
    ...
    fun requestPinAppWidget(widgetClassKey: String, promise: Promise) {
        try {
            if (isRequestPinAppWidgetSupported().not()) {
                promise.resolve(
                    Arguments.createMap().apply { putString("message", "not supported") }
                )
                return
            }
            ...
            if (widgetClass == null) {
                promise.resolve(
                    Arguments.createMap().apply { putString("message", "widget not found") }
                )
                return
            }
            ...
            if (result) {
                promise.resolve(Arguments.createMap().apply { putString("message", "success") })
            } else {
                promise.resolve(
